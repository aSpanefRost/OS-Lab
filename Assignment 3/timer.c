...
{
  // Rejects 0 or negative sleep ticks
  if (ticks<=0)
    return;
  int64_t start = timer_ticks ();

  // checking if the interrupts are ON
  ASSERT (intr_get_level () == INTR_ON);
  // disable interrupts to have timer_sleep() atomicity
  intr_disable();
  // get current thread using thread_current()
  struct thread * current_thread = thread_current();
  // add sleep time to the thread element 
  current_thread->sleep_time = start + ticks;
  // insert current thread into sorted list i.e. sleeping_threads list with threads of less sleep time first.
  list_insert_ordered(&sleeping_threads, &current_thread->sleep_elem, lesser_sleep, NULL);
  // blocks the current thread (putting it into equivalent of waiting state)
  thread_block();
  // enables interrupts
  intr_enable();
}
